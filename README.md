...
# Ollama translator patch

This fork adds the ability to translate in another languages than English, using an external Ollama instance.

### How to use ?

1. Make your own copy of the ".env.example" file called ".env" and add your values to it.
2. If you want Subgen to run on CPU, replace the "docker-compose.yml" file with a copy of "docker-compose.yml.example-cpu.yml". Skip this step if you want to keep the default one for GPU.
3. Run the docker container

### My setup

* Jellyfin webhooks only (no Bazarr/Plex/Emby/Tautulli).
* This Subgen Docker container.
* The following Ollama Docker service :
...
#### Multiples Docker Compose Subgen services

I use four docker services to run four instances of subgen :

1. __subgen-transcribe__ : Transcribe to audio language. Uses built-in WhisperAI.
2. __subgen-translate-en__ : Translate to English. Uses built-in WhisperAI.
3. __subgen-translate-mk__ : Translate to Macedonian. Uses an external Ollama instance.
4. __subgen-translate-fr__ : Translate to French. Uses an external Ollama instance.
...
Examples :
* "_I need my native language subtitles for this japanese anime_" --> Use subtitle-translate-mk
* "_English subtitles are enough for this regular british TV show_" --> Use subtitle-transcribe
* "_Let's learn Spanish with an Spanish video training_" --> Use subtitle-transcribe
* "_Let's watch a Italian video together and settle for english subtitles_" --> Use subtitle-translate-en

### Fixes from original project

* Fix timestamps in runtime image.

> See changes in Dockerfile & Dockerfile.cpu files.
...
* Added the ability to translate using a external Ollama instance.

* Dockerfile :
  * Rebuilt with "steps" to minimize build operations at code changes.
  * tzdata package included in the runtime image.
  * Added the ability to choose timezone with the Docker environment variable "TZ".

* Docker Compose files :
  * GPU file is ready for multi-GPU systems.
  * 4 subgen instances are used across 4 different services, each with a different purpose.

> See note in docker-compose.yml* files.

* Docker Compose environment files splitted

> See note in subgen-*.env files & docker-compose.yml.

* Docker Compose environment file ".env" used for custom secret values.

> Added to .gitignore, make your own copy from ".env.example" file.

* New Docker variables available :
  * TZ
  * OLLAMA_TRANSLATE
  * OLLAMA_TRANSLATE_API_URL
  * OLLAMA_TRANSLATE_SRC
  * OLLAMA_TRANSLATE_TGT
  * OLLAMA_TRANSLATE_MODEL

> See var notes in subgen-*.env files.

### Notes

* Original code is left untouched, as backup "*.original" files and commented lines in the source code.
* Ollama translation code was placed into a separate "translator_ollama.py" file.
* Tags used in the code :
  * "TRANSLATOR OLLAMA PATCH": in every code change
  * "# orginal code #": original code commented
  * "Bug": Bugs found
  * "Fix": Fix details
* Useful one-liner to rebuild & restart the docker container after code change :
```
docker compose down -v && docker compose up -d --build && docker compose logs -f
```

This is probably not the best integration, but I just did this for fun, and it works great for my use so far. Feel free to improve :)

[@McCloudS](https://github.com/McCloudS/subgen) : Lots of thanks for sharing your awesome work <3.
